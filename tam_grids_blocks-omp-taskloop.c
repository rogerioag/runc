...
void func(){
	int id;
	#pragma omp taskloop num_tasks(n_threads) shared(countConfig) private(id,bz, by, bx, gx, gy, gz) firstprivate(confGrid, confBlock, config)
...
int main(int argc, char const *argv[]){
    ...
    #pragma omp parallel num_threads(n_threads) 
    {
      ...
      #pragma omp single
      {
      	printf("Thread[%d][%lu]: Executing func...\n", id, (long int)pthread_self());
	      func();
	    }
	  }
